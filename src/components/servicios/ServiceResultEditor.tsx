...
import { CheckCircle2, FileDown, FlaskConical, Loader2, Save } from 'lucide-react';
import type { StudyResult, StudyResultValue } from '@/actions/results/resultsActions';
import { updateStudyResult } from '@/actions/results/resultsActions';
import type { StudyDetail } from '@/actions/studies/studiesActions';
import { formatDateTime, toApiDateTime, toDateTimeLocalInput } from '@/helpers/date';
...
export default function ServiceResultEditor({
  ...
  const [sampleAt, setSampleAt] = useState(toDateTimeLocalInput(initialResult.sampleAt));
  const [reportedAt, setReportedAt] = useState(toDateTimeLocalInput(initialResult.reportedAt));
  ...
  const [values, setValues] = useState<ResultFormValue[]>(mapResultValues(initialResult.values));
  ...
  const groupedRows = useMemo(() => {
    const categories = studyDetails
      .filter((detail) => detail.dataType === 'category' && detail.isActive !== false)
      .sort((a, b) => a.sortOrder - b.sortOrder);
    const categoryGroups = categories.map((category) => ({
      id: `category-${category.id}`,
      label: category.name,
      rows: values.filter((value) => detailById.get(value.studyDetailId ?? -1)?.parentId === category.id),
    }));
  ...
  const saveResult = async (mode: 'draft' | 'final') => {
    ...
    const payload = {
      ...
      reportedAt:
        mode === 'final'
          ? toApiDateTime(reportedAt || toDateTimeLocalInput(new Date().toISOString()))
    ...
    toast.success(mode === 'final' ? 'Resultado cerrado con exito.' : 'Borrador guardado.');
    setSavingMode(null);
  };
  ...
  const renderRows = (rows: ResultFormValue[]) => {
    return rows.map((value) => (
      ...
      >
        <div>
          <p className="text-xs font-semibold uppercase tracking-wide text-gray-500">Parametro</p>
          <input
            value={value.label}
        ...
        <div>
          <p className="text-xs font-semibold uppercase tracking-wide text-gray-500">Unidad</p>
          <input
            value={value.unit}
            onChange={(e) => handleValueChange(value.id, 'unit', e.target.value)}
            className="mt-2 w-full rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm text-gray-900 outline-none transition-all focus:border-red-500 focus:ring-2 focus:ring-red-500/20"
          />
        </div>
        <div>
          ...
          <textarea
            ...
            onChange={(e) => handleValueChange(value.id, 'referenceValue', e.target.value)}
        ...
        <label className="flex items-center justify-center gap-2 rounded-xl border border-gray-200 bg-gray-50 px-3 py-2 text-sm font-medium text-gray-700">
          <input
            ...
            onChange={(e) => handleValueChange(value.id, 'visible', e.target.checked)}
          />
          Visible
        </label>
      </div>
      ...
      <div className="flex flex-col gap-4 border-b border-gray-200 pb-4 lg:flex-row lg:items-start lg:justify-between">
        <div>
          <div className="flex flex-wrap items-center gap-2">
            <h3 className="text-lg font-semibold text-gray-900">{serviceItem.studyNameSnapshot}</h3>
            ...
            <span
              className={`rounded-full px-2.5 py-1 text-xs font-medium ${
                result.isDraft ? 'bg-amber-100 text-amber-700' : 'bg-emerald-100 text-emerald-700'
              }`}
            >
          ...
          <p className="mt-2 text-sm text-gray-500">
            Captura los valores del estudio, ajusta unidades y define que parametros saldran en el PDF.
          </p>
        </div>
      ...
      <div className="mt-5 grid gap-4 lg:grid-cols-[0.9fr_1.1fr]">
        <div className="space-y-4">
          <div className="rounded-2xl border border-gray-200 bg-gray-50 p-4">
            <p className="text-sm font-semibold text-gray-900">Metadatos del resultado</p>
          ...
          <div className="rounded-2xl border border-gray-200 bg-slate-900 p-4 text-white">
            ...
            <div className="mt-3 space-y-2 text-sm text-slate-200">
              ...
              <div className="flex items-center justify-between">
                <span>Reporte</span>
                <span>{reportedAt ? formatDateTime(toApiDateTime(reportedAt)) : 'Sin cerrar'}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-4">
          ...
          {groupedRows.uncategorized.length > 0 ? (
            <div className="rounded-2xl border border-amber-200 bg-amber-50 p-4">
              <p className="mb-3 text-sm font-semibold text-amber-900">Parametros sin categoria</p>
              <div className="space-y-3">{renderRows(groupedRows.uncategorized)}</div>
            </div>
          ) : null}
