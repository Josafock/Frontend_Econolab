...
export default function PatientDetailPage() {
  ...
  const age = useMemo(() => calculateAge(formData.fechaNacimiento), [formData.fechaNacimiento]);
  ...
  const handleCancelEdit = () => {
    ...
    setTouched({});
    setIsEditing(false);
    router.replace(`/pacientes/detalle/${id}`);
  };
    ...
    <div className="p-8">
      <div className="mb-6 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div>
          ...
          <h1 className="mt-4 text-3xl font-bold text-gray-900">Detalle de paciente</h1>
          <p className="mt-2 text-gray-600">
            Consulta el expediente y realiza cambios sin perder el historial del registro.
          </p>
        </div>
            ...
            {isEditing ? (
              ...
              <>
                ...
                >
                  ...
                  {patient.isActive !== false ? 'Suspender paciente' : 'Reactivar paciente'}
                </button>
              </>
            )}
          </div>
          ...
          <div className="grid gap-4 lg:grid-cols-[1.2fr_0.8fr]">
            <div className="rounded-[2rem] border border-gray-200 bg-white p-6 shadow-sm">
              <div className="mb-4 flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-lg font-semibold text-gray-900">Resumen del expediente</h2>
                  <p className="text-sm text-gray-500">
                    Vista rápida del registro actual del paciente.
                  </p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                ...
                <div className="rounded-2xl border border-gray-200 bg-gray-50 p-4">
                  ...
                  <p className="mt-2 text-base font-semibold text-gray-900">{formatDocument(patient)}</p>
                </div>
                <div className="rounded-2xl border border-gray-200 bg-gray-50 p-4">
                  <p className="text-xs uppercase tracking-wide text-gray-500">Genero</p>
                  <p className="mt-2 text-base font-semibold text-gray-900">
                    {getGenderLabel(patient.gender)}
                  </p>
                </div>
                <div className="rounded-2xl border border-gray-200 bg-gray-50 p-4">
                  <p className="text-xs uppercase tracking-wide text-gray-500">Edad actual</p>
                    <p className="mt-2 text-base font-semibold text-gray-900">{age} años</p>
                </div>
                <div className="rounded-2xl border border-gray-200 bg-gray-50 p-4">
                  <p className="text-xs uppercase tracking-wide text-gray-500">Telefono</p>
                  <p className="mt-2 text-base font-semibold text-gray-900">{patient.phone ?? '-'}</p>
                </div>
                <div className="rounded-2xl border border-gray-200 bg-gray-50 p-4">
                  <p className="text-xs uppercase tracking-wide text-gray-500">Correo</p>
                  <p className="mt-2 text-base font-semibold text-gray-900">{patient.email ?? '-'}</p>
                </div>
              </div>
            </div>
            ...
            <div className="rounded-[2rem] border border-gray-200 bg-gradient-to-br from-red-600 via-red-500 to-rose-500 p-6 text-white shadow-lg shadow-red-600/20">
              ...
              <div className="mt-6 space-y-4 rounded-[1.5rem] bg-white/10 p-5 backdrop-blur-sm">
                <div>
                  <p className="text-xs uppercase tracking-wide text-red-100">Dirección</p>
                  <p className="mt-1 text-sm font-medium text-white">
                    {patient.addressLine ?? 'Sin direccion'}
                  </p>
                </div>
                <div>
                  <p className="text-xs uppercase tracking-wide text-red-100">Entre calles</p>
                  <p className="mt-1 text-sm font-medium text-white">
                    {patient.addressBetween ?? 'Sin referencia'}
                  </p>
                </div>
                <div>
                  <p className="text-xs uppercase tracking-wide text-red-100">Ubicacion</p>
                  <p className="mt-1 text-sm font-medium text-white">
                    {[patient.addressCity, patient.addressState, patient.addressZip]
                      .filter(Boolean)
                      .join(', ') || 'Sin ubicación'}
                  </p>
                </div>
              ...
              <div>
                ...
                <p className="text-sm text-gray-500">
                  {isEditing
                    ...
                    : 'Consulta toda la información capturada del paciente.'}
                </p>
              </div>
              {!isEditing && (
                <button
