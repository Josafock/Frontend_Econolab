...
export async function GET(_: Request, context: RouteContext) {
  ...
  const apiUrl = `${process.env.API_URL}/services/${id}/labels`;
  ...
  if (!upstream.ok) {
    ...
    return NextResponse.json(
      { message: payload || 'No se pudo generar el codigo de barras.' },
  ...
  return new NextResponse(pdf, {
    ...
    headers: {
      ...
      'Content-Disposition': `inline; filename="etiquetas-${id}.pdf"`,
    },
  });
}
